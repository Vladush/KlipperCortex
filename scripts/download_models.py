...
def main():
    ...
    parser.add_argument(
        "--model",
        choices=["1", "2", "3"],
        help="Select model to download (1=Official ONNX, 2=Coral, 3=Obico Legacy)",
    )
    ...
    if model_info["type"] == "onnx" and os.path.exists(
        os.path.join(MODEL_DIR, "model.tflite")
    ):
        print(
            "Note: You have both model.onnx and model.tflite. The compiler will prioritize ONNX if you pass it."
        )
    elif model_info["type"] == "tflite" and os.path.exists(
        os.path.join(MODEL_DIR, "model.onnx")
    ):
        print(
            "Note: You have both model.onnx and model.tflite. The compiler will prioritize TFLite if you pass it."
        )
