...
def load_connections(config_path):
    if not os.path.exists(config_path):
        ...
        print("Please copy 'connections.json.example' to 'connections.json' and configure your devices.")
        sys.exit(1)
...
def deploy(device, model_path, inference_script):
    ...
    scp_cmd = ["scp"] + ssh_opts + [inference_script, f"{user}@{host}:{target_dir}/inference_loop.py"]
    run_command(scp_cmd)
    ...
    ssh_cmd = ["ssh"] + ssh_opts + [f"{user}@{host}", f"sudo systemctl restart {service_name}"]
    # We use 'run_command' but catch errors in case sudo needs a password or service doesn't exist yet
    try:
        subprocess.check_call(ssh_cmd)
        print("  Service restarted successfully.")
    except subprocess.CalledProcessError:
        print("  Warning: Failed to restart service. It might not be installed or requires sudo password.")

    print(f"Deployment to {device.get('name')} complete.\n")

def main():
    parser = argparse.ArgumentParser(description="Deploy KlipperCortex to remote printers.")
    parser.add_argument("--config", default="connections.json", help="Path to connections configuration file.")
    parser.add_argument("--model", default="models/spaghetti_v2.vmfb", help="Path to compiled .vmfb model.")
    parser.add_argument("--script", default="src/inference_loop.py", help="Path to inference script.")
